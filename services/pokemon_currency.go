...
func (s *Service) GetRegistries(odd bool, itemsNumber int, workers int, pokemons []*models.Pokemon) ([]*models.Pokemon, error) {
	...
	for i := 0; i < numberOfWorkers; i++ {
		go s.getPokemonsByRules(jobs, results, wg, odd, len(pokemons))
	}
	go func() {
		wg.Wait()
		close(results)
...
func (s *Service) getPokemonsByRules(jobs <-chan models.Pokemon, results chan<- models.Pokemon, wg *sync.WaitGroup, odd bool, pokemonsQuantity int) {
